		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/course_selection/table','add') || $check_action('/course_selection/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/course_selection/table','del') || $check_action('/course_selection/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','course_name')" min-width="200">
					...
					<el-table-column prop="experimental_arrangement" @sort-change="$sortChange" label="实验安排"
				v-if="user_group == '管理员' || $check_field('get','experimental_arrangement')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/course_selection/table','set') || $check_action('/course_selection/view','set') || $check_action('/course_selection/view','get') || $check_action('/签到/table','add') || $check_action('/签到/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/course_selection/table','set') || $check_action('/course_selection/view','set') || $check_action('/course_selection/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/student_check_in/view')" v-if="user_group == '管理员' || $check_action('/student_check_in/table','add') || $check_action('/student_check_in/view','add')">
						<span>签到</span>
					</el-button>
							</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/course_selection/get_list?like=0",
				url_del: "~/api/course_selection/del?",
