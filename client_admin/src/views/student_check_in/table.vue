		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/student_check_in/table','add') || $check_action('/student_check_in/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/student_check_in/table','del') || $check_action('/student_check_in/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				v-if="user_group == '管理员' || $check_field('get','submission_time')" min-width="200">
					...
					<el-table-column prop="confirm_attendance" @sort-change="$sortChange" label="确认到场"
				v-if="user_group == '管理员' || $check_field('get','confirm_attendance')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/student_check_in/table','set') || $check_action('/student_check_in/view','set') || $check_action('/student_check_in/view','get') || $check_action('/添加考勤/table','add') || $check_action('/添加考勤/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/student_check_in/table','set') || $check_action('/student_check_in/view','set') || $check_action('/student_check_in/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/check_work_attendance/view')" v-if="user_group == '管理员' || $check_action('/check_work_attendance/table','add') || $check_action('/check_work_attendance/view','add')">
						<span>添加考勤</span>
					</el-button>
							</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/student_check_in/get_list?like=0",
				url_del: "~/api/student_check_in/del?",
