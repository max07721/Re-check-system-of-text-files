	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="教师账号" prop="teacher_account">
							...
							<!--v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','teacher_account')) || (!form['experimental_information_id'] && $check_field('add','teacher_account'))" :disabled="disabledObj['teacher_account_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','teacher_account')">{{form['teacher_account']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','teacher_account')) || (!form['experimental_information_id'] && $check_field('add','teacher_account'))" id="teacher_account" v-model="form['teacher_account']" :disabled="disabledObj['teacher_account_isDisabled']">
				...
				<el-form-item label="教师姓名" prop="teachers_name">
							  ...
							  v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','teachers_name')) || (!form['experimental_information_id'] && $check_field('add','teachers_name'))" :disabled="disabledObj['teachers_name_isDisabled']"></el-input>
				...
				<el-form-item label="学生账号" prop="student_account">
													<el-select v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','student_account')) || (!form['experimental_information_id'] && $check_field('add','student_account'))" id="student_account" v-model="form['student_account']" :disabled="disabledObj['student_account_isDisabled']">
				...
				<el-form-item label="学生姓名" prop="student_name">
								<el-select id="student_name" v-model="form['student_name']"
						v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','student_name')) || (!form['experimental_information_id'] && $check_field('add','student_name'))">
				...
				<el-form-item label="课程名称" prop="course_name">
						...
						v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','course_name')) || (!form['experimental_information_id'] && $check_field('add','course_name'))">
				...
				<el-form-item label="实验题目" prop="experimental_topic">
								<el-select id="experimental_topic" v-model="form['experimental_topic']"
						v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','experimental_topic')) || (!form['experimental_information_id'] && $check_field('add','experimental_topic'))">
				...
				<el-form-item label="实验地点" prop="experimental_location">
							  ...
							  v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','experimental_location')) || (!form['experimental_information_id'] && $check_field('add','experimental_location'))" :disabled="disabledObj['experimental_location_isDisabled']"></el-input>
				...
				<el-form-item label="实验内容" prop="experimental_content">
						...
						v-if="user_group === '管理员' || (form['experimental_information_id'] && $check_field('set','experimental_content')) || (!form['experimental_information_id'] && $check_field('add','experimental_content'))" :disabled="disabledObj['experimental_content_isDisabled']"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/experimental_information/add?",
				url_set: "~/api/experimental_information/set?",
				url_get_obj: "~/api/experimental_information/get_obj?",
				url_upload: "~/api/experimental_information/upload?",

				query: {
					"experimental_information_id": 0,
				},
				...
				form: {
								"teacher_account": 0, // 教师账号
										...
										"experimental_content":  '', // 实验内容
											"experimental_information_id": 0, // ID
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/experimental_information/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/experimental_information/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/experimental_information/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/experimental_information/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/experimental_information/view','get');
