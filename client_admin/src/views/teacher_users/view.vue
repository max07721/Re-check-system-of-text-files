			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="教师姓名" prop="teachers_name">
					...
					<div v-else-if="$check_field('get','teachers_name')">{{form_sub['teachers_name']}}</div>
								...
								<el-col v-if="user_group === '管理员' || $check_field('get','teacher_id') || $check_field('add','teacher_id') || $check_field('set','teacher_id')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教师工号" prop="teacher_id">
												<el-input id="teacher_id" v-model="form_sub['teacher_id']" placeholder="请输入教师工号"
							  v-if="user_group === '管理员' || (form_sub['teacher_users_id'] && $check_field('set','teacher_id')) || (!form_sub['teacher_users_id'] && $check_field('add','teacher_id'))"></el-input>
					<div v-else-if="$check_field('get','teacher_id')">{{form_sub['teacher_id']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','gender_of_teacher') || $check_field('add','gender_of_teacher') || $check_field('set','gender_of_teacher')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教师性别" prop="gender_of_teacher">
								<el-select id="gender_of_teacher" v-model="form_sub['gender_of_teacher']"
						v-if="user_group === '管理员' || (form_sub['teacher_users_id'] && $check_field('set','gender_of_teacher')) || (!form_sub['teacher_users_id'] && $check_field('add','gender_of_teacher'))">
						<el-option v-for="o in list_gender_of_teacher" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','gender_of_teacher')">{{form_sub['gender_of_teacher']}}</div>
							</el-form-item>
			</el-col>
			...
			return {
				...
				disabledObj:{
								"teachers_name_isDisabled": false,
										...
										"gender_of_teacher_isDisabled": false,
							},

				form_sub: {
								"teachers_name":  '', // 教师姓名
										"teacher_id":  '', // 教师工号
										"gender_of_teacher":  '', // 教师性别
									    "user_id": 0,
