			...
			<el-row>
				<el-col>
					<el-form :model="form" :rules="rules" ref="form" label-width="60px" class="form">
						<h1>Text duplication checking </h1>
						
						<el-form-item label="Account" prop="account">
							<el-input type="text" v-model="form.account" placeholder="Username / mobile phone / email">
							</el-input>
						</el-form-item>
						
						<el-form-item label="Password" prop="password">
							<el-input type="password" v-model="form.password" placeholder="5 to 16 characters"
								autocomplete="off">
							</el-input>
						</el-form-item>
						<!-- 极验开始 -->
						<div class="geetest_box">
							<div class="geetest_box_title">Verify</div>
						...
						<!-- 极验结束 -->
						<div class="btns">
							<el-button type="primary" @click="sign_in()">Login</el-button>
                            <router-link class="el-button el-button--primary" to="./register">
								Register
							</router-link>
						</div>
						<div class="btns">
							<router-link to="./forgot" > Forget?</router-link>
						</div>
					</el-form>
				</el-col>
			</el-row>
	...
	export default {
				...
				rules: {
					account: [{
						...
						},
						{
							min: 5,
							max: 16,
							message: '5 to 16 characters in length',
							trigger: 'blur'
						}
					],
					password: [{
		...
		methods: {
			...
			slidePictureState(state) {
						if (state === true) {
							// 成功
							this.$message.success("Wow~ The test was successful, the speed beats 99% of users, please log in");
			...
			sign_in() {
				...
				if(!this.form.account){
					return this.$message.warning("Please enter the correct account number")
				}else if(!this.form.password){
					return this.$message.warning("Please enter correct password")
				}
				// 极验开始
				else if(!this.isGeeFinish){
					return this.$message.warning("Please perform drag and drop test")
				}
				// 极验结束
				var _this = this;
